...
class ReferenceCatalogTest(TestCase):
    ...
    def setUpClass(self):
        ...
        refpath = Path('data/testdata/EPOCH08/COMBINED/STOKESI_SELAVY/VAST_0012+00A.EPOCH08.I.selavy.components.txt')
        self.racscat = ReferenceCatalog(racspath)
        self.refcat = ReferenceCatalog(refpath)

    def test_filepath_as_str(self):
        racscat = ReferenceCatalog('data/testdata/RACS_catalogue_test.fits')
...
class CatalogTest(TestCase):
    ...
    def test_nvss(self):
        ...
        self.assertEqual(len(catalog.sources), 2853)

    def test_racs(self):
        catalog = Catalog(self.image, 'racs')
        self.assertEqual(len(catalog.sources), 505)
